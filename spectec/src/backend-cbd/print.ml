...
let rec make_index depth =
  List.init (depth+1) (fun _ -> "    ") |> String.concat ""
  (* _index := !_index + 1; *)
...
and string_of_stack_prefix _ = "stack prefix"

and string_of_instr' depth instr =
  let indent = "  " in
  ...
  let open Al.Ast in
  match instr.it with
  | PopI {it = CaseE ([{ it=Atom.Atom ("CONST" | "VCONST"); _ }]::_tl, hd::tl); _} ->
    sprintf "%s def %s = pop_%s()" (make_index depth) (string_of_exprs " " tl) (string_of_expr hd)
  | PopI e ->
    sprintf "%s def %s = pop_Value()" (make_index depth) (string_of_expr e)
  (* | PopI e -> *)
  ...
  | IfI (e, il1, il2) ->
    ...
    let if_prose = sprintf "%s if (%s) {%s}" if_index (string_of_expr e)
  ...
  | PopAllI e ->
    sprintf "%s Pop all values %s from the top of the stack." (make_index depth)
      (string_of_expr e)
  | LetI (e1, e2) ->
    sprintf "%s def %s = %s." (make_index depth) (string_of_expr e1)
      (string_of_expr e2)
  | TrapI -> sprintf "%s doTrap()\n%s" (make_index depth) (make_index (depth-1))
  ...
  | ExecuteI e ->
    sprintf "%s Execute the instruction %s.\n%s" (make_index depth) (string_of_expr e) (make_index depth)
  | ExecuteSeqI e ->
...
let include_instr instr = match instr.it with
| AssertI _ -> false
| _ -> true
...
| AlgoD { it = RuleA (_, _, _, _); _ } -> true
| _ -> false

let string_of_cbd cbd = List.filter is_step cbd |> List.map string_of_def |> String.concat "\n"
